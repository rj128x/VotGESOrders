...
namespace VotGESOrders.Web.Models
{
	public class OrderView
	{
		public static string getOrderHTML(Order order) {
			...
			string htmlNumber = String.Format("Заявка {0} №{1} от {2}",order.OrderTypeShortName, order.OrderNumber, order.OrderDateCreate.ToString("dd.MM.yy"));
			...
			string htmlExtend=order.OrderHasParentOrder ? "СТАРАЯ ЗАЯВКА №" + order.ParentOrderNumber.ToString() : "";
			string htmlExtended=order.OrderHasChildOrder ? "НОВАЯ ЗАЯВКА №" + order.ChildOrderNumber.ToString() : "";			
